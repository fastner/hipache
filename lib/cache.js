...
function Cache(config, handlers) {
    ...
    this.subRedisClient = redis.createClient(
            this.config.redisPort,
            this.config.redisHost
            );
    ...
    if (this.config.redisDatabase) {
        ...
        this.subRedisClient.select(this.config.redisDatabase);
    }       
    
    //auth redis
    if (this.config.redisPassword) {
        this.redisClient.auth(this.config.redisPassword);
        this.subRedisClient.auth(this.config.redisPassword);
    }             
    
    this.redisClient.on('error', function (err) {
        this.log('RedisError ' + err);
    }.bind(this));
    this.subRedisClient.on('error', function (err) {
        this.log('RedisError ' + err);
    }.bind(this));
...
/*
 * This method calls callback if SNI configuration is changed 
 * and published via REDIS pubsub message
 */
Cache.prototype.watchSNIConfig = function (callback) {
    var channelName = "sni configuration";
    var self = this;
    this.subRedisClient.on("message", function(channel, message) {
        if (channel == channelName && message == "reload") {
            self.getSNIConfig(callback);
        }
    });
    this.subRedisClient.subscribe(channelName);
};

/*
 * This method gets current SNI configuration.
 */
Cache.prototype.getSNIConfig = function (callback) {
    this.redis.keys('sni:*', function(err, values) {
